...
watch(
  () => props.payload,
  (newValue) => {
    CFLB2.title = newValue.title
    CFLB2.last_modified = newValue.last_modified
    CFLB2.containers = newValue.containers
    CFLB2.cards = newValue.cards
  },
  { immediate: true }
)
watch(
...
const cardList = (containerId) => {
  return CFLB2.cards.filter((card) => card.id_container === containerId)
}
...
const dropItem = (event, containerId) => {
  ...
  const item = CFLB2.cards.find((card) => card.id == id)
  item.id_container = containerId
  CFLB2.last_modified = new Date().toLocaleString('en-GB')
  store.dispatch('CFLB2/setCFLB2', CFLB2)
}
...
const deleteItem = (type) => {
  if (type === 'container') {
    CFLB2.containers = CFLB2.containers.filter(
      (container) => container.id !== state.selectedContainerId
    )
    CFLB2.cards = CFLB2.cards.filter(
...
const handleKanbanAction = (mode, type, containerId, payload) => {
  if (mode === 'add') {
    if (type === 'container') {
      const newContainer = {
        id:
          CFLB2.containers.length > 0
            ? [...CFLB2.containers].pop().id + 1
            : 1,
        name: newContainerTitle.value,
      }
      CFLB2.containers.push(newContainer)
      state.isAddingContainer = false
    } else {
      newCardData.id =
        CFLB2.cards.length > 0 ? [...CFLB2.cards].pop().id + 1 : 1
...
const handleEditCard = (type, selectedCard) => {
  if (type === 'change') {
    ...
    state.tempCards = CFLB2.cards.map((card) => ({ ...card }))
  } else if (type === 'save') {
...
<template>
  ...
  >
    <TransitionGroup name="list">
      <div
        v-for="container in CFLB2.containers"
        :key="container.id"
        class="mx-1"
      >
        <div
