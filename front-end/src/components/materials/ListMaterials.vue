	...
	<div class="w-300 flex flex-col gap-4 h-full">
		<div class="w-full">
			...
			<div class="relative w-fit">
				<div>
					<div class="h-3/6 absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
						<svg
							aria-hidden="true"
					...
					<button
						type="submit"
						class="text-white absolute right-2.5 bottom-16 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
						@click="filteredList()"
					>
						Search
					</button>
					<div class="item error" v-if="input && !materials.length">
				...
				<div class="w-fit mt-4 flex flex-row items-center justify-between">
					<select
						id="selectSite"
						class="w-100 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-100 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
						v-model="site"
					>
