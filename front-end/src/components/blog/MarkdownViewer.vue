    ...
    <div class="codeLab text-gray-900 dark:text-white ">
      ...
      <div v-if="formatedArray.length>0" class=" mb-2 tracking-tight text-gray-900 dark:text-white">
        ...
        <div tabindex="-1"
          ...
          <div class="flex items-center mx-auto">
            ...
            <button
                ...
                @click.prevent="toggle()" type="button"
        ...
        <div class="flex flex-row">
          <div :class=" windowWidth >= 1024 ? 'flex flex-col items-center w-3/4' : 'flex flex-col items-center w-4/4'"> <!-- first div of new encapsulation -->
            <div class="flex flex-row">
              <div>
                <div v-if="step===0"
                     class="mt-12 block max-w-xl min-w-[526px] p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                ...
                <div
                    class="block articleContent min-w-[526px] max-w-xl p-6 mt-6 mb-2 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                ...
                <!-- formatedArray.length-1 instead of -2 on nextBTN ICI LA CEST LA QUON TAFF-->
                <div class="flex flex-row justify-between"
                :class="step == 0 ? 'flex-row-reverse' : ''">
                  <button v-if="step>0" @click="step = step -= 1"
                          class="backBtn py-2.5 px-5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white ">
                    Back
                  </button>
                  <button v-if="step<formatedArray.length-1" @click="step=step=step+=1"
                          :class="step == 0 ? 'flex-end' : ''"
                          class="nextBtn text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none">
                    Next
                  </button>
                </div>
              </div>
            ...
            <div class="h-80 px-3 py-4 overflow-y-auto bg-transparent transition-transform h-full">
              <ul class="space-y-2 font-medium">
                <li v-for="(item, index) in headers" :key="index">
                  ...
                  >
                    <div class="flex flex-row">
                      <div class="flex self-center min-w-[25.5px] rounded-full justify-center text-gray-200"
                           :class="step >= index ? 'bg-blue-600 dark:bg-blue-800' : 'bg-gray-500 dark:text-black '">
                        {{ index + 1 }}
                      </div>
...
if (!props.markdown) {
  ...
  onMounted(async () => {
    ...
    await materialStore.getUserById(oneItems.value.owner).then((res) => {
      console.log(res)
      author.value = res.profile.firstName + " " + res.profile.lastName;
    });
