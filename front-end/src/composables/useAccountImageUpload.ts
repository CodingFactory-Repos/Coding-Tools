...
import { UserBusinessProfile, UserProfile, UserSchoolProfile } from "@/store/interfaces/auth.interfaces";
...
export function useAccountImageUpload() {
	...
	const imageUploaded = (url: string) => {
		...
		if(!userStore.temporaryProfileUser[picturePath.value.profile]) {
			userStore.temporaryProfileUser[picturePath.value.profile] = {} as UserProfile & UserBusinessProfile & UserSchoolProfile;
		}
