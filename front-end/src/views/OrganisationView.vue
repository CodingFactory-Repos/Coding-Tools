<template>
	<div
		class="mx-auto p-4 rounded-lg bg-light-primary dark:bg-dark-secondary text-left mt-10 ml-12 mr-12 w-11/12"
	>
		<div v-show="showProject == ''">
			<h1 class="text-6xl font-bold dark:text-dark-font">Organisation</h1>
			<div class="actualSprint marg">
				Sprint sélectionné:<br />
				...
				<div class="groups">
					<div class="iconeGroupe" v-for="group in groups">
						...
						<div style="position: absolute; left: 10px; bottom: 10px">
							<button @click="openProject(group)" class="rounded-lg hover:bg-stone-300">
								Ouvrir le projet
							</button>
						</div>
					</div>
		...
		<div v-show="showProject" class="ml-12">
			...
			<div
				class="w-full justify-center h-fit rounded-lg bg-light-primary dark:bg-dark-tertiary py-4 px-4"
				v-if="courseId != ''"
			>
				<h1 class="dark:text-[#9ca3af] text-[#213547]">Membres</h1>
				<div class="flex flex-row flex-wrap gap-3 mt-4">
					<div
						class="bg-[#5C5F73] rounded-lg px-4 py-2"
						v-for="student in selectedGroup"
						:key="student"
					>
			...
			http.get(`/calls/student_list/${this.courseId}`).then((response) => {
				...
				this.groups.forEach((group) => {
					...
					group.forEach((student) => {
						//Pour chaque élève du groupe
						this.studentList.forEach((user) => {
							//On compare avec tous les élèves
							if (student == user._id) {
								group[group.indexOf(student)] =
